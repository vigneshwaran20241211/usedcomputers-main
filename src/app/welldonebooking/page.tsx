...
const Welldone = () => {
  const [formbookingData, setformbookingData] = useState(null);  // Initialize as null to handle loading state
  const fileUrl = formbookingData ? `http://dev.usedcomputer.com.my:3000/invoice/${formbookingData.bookingId}.pdf` : '';
  ...
  useEffect(() => {
    const storedData = sessionStorage.getItem("formbookingData");
    if (storedData) {
      setformbookingData(JSON.parse(storedData));  // Parse formbookingData when it exists in sessionStorage
    }
  }, []);

  // If formbookingData is still loading or not available, display a loading message
  if (!formbookingData) {
    return <div>Loading...</div>;
  }

const calculateMaterialTotals = (products, formbookingData) => {
  let materialTotals = {
    plastic: 0,
    copper: 0,
    aluminium: 0,
  ...
  // Loop through formbookingData products
  formbookingData.forEach(data => {
    // Loop through each product in formbookingData
...
// Example usage
const materialTotals = calculateMaterialTotals(category, formbookingData.category);
...
const handlePrint = () => {
  ...
  formbookingData.category.forEach(category => {
...
<title>Booking Invoice - Summery</title>
<style>
  body {
...
<div class='email-container'>
        ...
        <td align='center' valign='top' style='padding:0'>
          <table bgcolor='#FFFFFF' cellspacing='0' cellpadding='10' border='0' width='100%'>
            <tbody>
                      ...
                      <tr>
                        ...
                        <td valign='top' align='right'>
                          <h2 style='font-family:Arial,Helvetica,sans-serif;font-size:18px;margin:3px 0px'>Quotation
                            #:
                            <span style='color:#be1e31'>${formbookingData.bookingId}</span>
                          </h2>
                        </td>
                      </tr>
                    </tbody>
                        ...
                        <td valign='bottom'>
                          <div style='font-family:Arial,Helvetica,sans-serif;font-size:14px;margin-top:5px'>
                            ...
                            <p style='margin:0;margin-left:10px;margin-top:3px'><span
                                ...
                                Date: </span><span
                                style='font-family:Helvetica;font-size:14px;font-weight:600;margin:3px 0px'> ${formbookingData.bookingDate}</span>
                            </p>
                          </div>
                        </td>
              ...
              <tbody>
                ${formbookingData.category.map((category) => {
    return category.products.map((product) => {
      return `
                      <tr>
                        ...
                        <td style="text-align:center;">RM   ${formbookingData.total}</td>
                      </tr>
                    `;
    }).join('');
  }).join('')}
                      ...
                      <tr>
                        <td colspan='3' align='right' width='60%'>
                          <h3 style='font-family:Arial,Helvetica,sans-serif;font-size:14px;margin:3px 0px'>
                            Booking Type / Booking Session: &nbsp;</h3>
                        </td>
                        <td align='center' border='0' width='20%'>
                          <h3 style='font-family:Arial,Helvetica,sans-serif;font-size:14px;margin:3px 0px'>
                          ${formbookingData.bookingType} / ${formbookingData.bookingSession}
                          </h3>
                        </td>
                      </tr>
                       <tr>
                        ...
                        <td align='right' border='0' width='20%'>
                          <h3 style='font-family:Arial,Helvetica,sans-serif;font-size:14px;margin:3px 0px'>
                            RM
                            <!--${grandTotal}-->
                            ${formbookingData.totalPaid}
                          </h3>
                        </td>
                      </tr>
                      <tr>
                        <td colspan='3' align='right' width='60%'>
                          <h3 style='font-family:Arial,Helvetica,sans-serif;font-size:14px;margin:3px 0px'>Pick Up Cost: &nbsp;</h3>
                        </td>
                        <td align='right' border='0' width='20%'>
                          <h3 style='font-family:Arial,Helvetica,sans-serif;font-size:14px;margin:3px 0px'>
                            RM
                            <!--40-->
                            ${formbookingData.pickupCharge}
                          </h3>
                        </td>
                      </tr>
                      <tr>
  ...
  return (
    ...
    <h3 className="lg:text-40 xl:text-34 pb-4 text-base font-extrabold text-primary lg:pb-8 ordernum wellmarone">{formbookingData.bookingId}</h3>
    ...
    <div className="border-gradient relative p-6 lg:p-10 my-10 lg:my-20 wellmartwo">
      ...
      {/* <p className="text-primary font-semibold text-22 lg:text-28 xl:text-34 ordernum">
        You have just collected a total of <span className="text-secondary font-extrabold">{materialTotals.plastic + materialTotals.gold + materialTotals.copper + materialTotals.aluminium}g</span> of E-Waste. When correctly recycled, this saves up to <span className="text-secondary font-extrabold">10.3kg of CO2 equivalent.</span>
      </p> */}
      <p className="text-primary font-semibold text-22 lg:text-28 xl:text-34 ordernum">
