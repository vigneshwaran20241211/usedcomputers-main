...
const BookingHistory: React.FC<ParamsInterface> = (params: ParamsInterface) => {
    ...
    const fetchData = async (page: number) => {
        ...
        try {
            const result = await axios.get(`${process.env.API_URL}quotation/all?page=${page}&limit=${limit}`, {
                headers: { Authorization: `Bearer ${accessToken}` },
            });
            ...
            if (Array.isArray(result.data.data)) {
                ...
                result.data.data.forEach((quotation: any) => {
                    if (quotation.stateId && !stateNames[quotation.stateId]) {
                        fetchStateName(quotation.stateId);
                    }
                });
            } else {
                setStateData([]);
                ...
                <div className="container mx-auto">
                    <fieldset className="px-4 lg:px-0">
                        <div className="basic-detail">
                            <h3 className="text-primary uppercase text-22 lg:text-64 xl:text-64 pb-4 lg:pb-8 text-center">Quotation History</h3>
                                ...
                                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                    <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                        <tr>
                                            ...
                                            <th scope="col" className="px-6 py-3">Download Quotation</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {stateData.map((quotation, index) => (
                                            <tr
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-4">{quotation.quotationId}</td>
                                                <td className="px-6 py-4">
                                                    {stateNames[quotation.stateId] || "No State..."} {/* Show state name or loading message */}
                                                </td>
                                                
                                                <td>
                                                    {quotation.category[0]?.products?.map((product, i) => (
                                                        <div key={i}>
                                                            {product.title} - Quantity: {product.quantity} - Price: RM{product.price}<br></br>
                                                        </div>
                                                    ))}
                                                </td>
                                                
                                                <td className="px-6 py-4">{quotation.quotationType}</td>
                                                <td className="px-6 py-4">{quotation.quotationDate}</td>
                                                <td className="px-6 py-4">{quotation.quotationSession}</td>
                                                <td className="px-6 py-4">{quotation.status}</td>
                                                <td className="px-6 py-4">
                                                    <Link href={`http://apidev.usedcomputer.com.my:3000/quotation/${quotation.quotationId}.pdf`} target="blank"
