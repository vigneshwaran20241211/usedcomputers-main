...
const TableTwo: React.FC = (params: paramsInterface) => {
  ...
  const fetchData = async (page: number) => {
    ...
    const accessToken = localStorage.getItem('accessToken');
    try {
      const result = await axios.get(`${process.env.API_URL}user?page=${currentPage}&limit=${limit}&term=CC`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
...
<td className="px-6 py-4 flex flex-col space-y-2">
  ...
  <div className="flex items-center space-x-2">
    <span>{user.dic}</span>
    <a href={user.dicFile} target="_blank" className="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600">
      Director
    </a>
  </div>
  <br></br>
  <div className="flex items-center space-x-2">
    <span>{user.ml}</span>
    <a href={user.mlFile} target="_blank" className="bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600">
      Municipal
    </a>
  </div>
</td>
