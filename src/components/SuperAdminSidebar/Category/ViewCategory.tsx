...
const ViewproductCategory: React.FC = (params: paramsInterface) => {
  ...
  const handleStatus = (status, id) => {
    ...
    try {
      ...
      axios.put(
        `${process.env.API_URL}category/status/${id}`, // Ensure URL is correct and consistent
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setLoading(true);
      const response = await axios.delete(`${process.env.API_URL}category/${id}`, {
  ...
  const fetchData = async (page: number) => {
    ...
    const accessToken = localStorage.getItem('accessToken');
    try {
      const result = await axios.get(`${process.env.API_URL}category/all?page=${currentPage}&limit=${limit}`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
  ...
  return (
    <div className="rounded-[10px] bg-white shadow-1 dark:bg-gray-dark dark:shadow-card">
      ...
      <div className="px-4 py-6 md:px-6 xl:px-9">
        ...
        <Link href="/superadmin/master/addcategory">
          <button
            type="button"
          ...
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
              <tr>
                ...
                <th scope="col" className="px-6 py-3">Country Image</th>
                <th scope="col" className="px-6 py-3">Status</th>
                <th scope="col" className="px-6 py-3">Action</th>
              </tr>
            </thead>
            <tbody>
              {stateData.map((category, index) => (
                <tr
                  key={category.id || index} // Prefer country.id if available, fallback to index
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <Image
                      src={category.image || "/images/default-image"} // Use default-image from the root
                      alt="Category Image"
                      width={100}
                      height={100}
                  ...
                  <td className="px-6 py-4">
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        onChange={() => handleStatus(category.status, category.id)}
                        className="sr-only peer"
                        defaultChecked={category.status === "ACTIVE"} // Set defaultChecked based on status
                      />
                      <div
    ...
    peer-checked:bg-blue-500"
                  ...
                  <td className="px-6 py-4">
                    <Link
                      href={`/superadmin/master/addcategory?id=${category.id}`}
                    ...
                    <button onClick={() => handleDelete(category.id)} className="mt-10 text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 focus:outline-none dark:focus:ring-red-800">
                      Delete
                    </button>
                  </td>
                </tr>
