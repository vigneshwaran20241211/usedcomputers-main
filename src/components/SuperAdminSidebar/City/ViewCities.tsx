...
const TableTwo: React.FC = (params: paramsInterface) => {
  ...
  const handleStatus = (status, id) => {
    ...
    try {
      ...
      axios.put(
        `${process.env.API_URL}city/status/${id}`, // Ensure URL is correct and consistent
  ...
  const fetchData = async (page: number) => {
    ...
    const accessToken = localStorage.getItem('accessToken');
    try {
        const result = await axios.get(`${process.env.API_URL}city/all?page=${currentPage}&limit=${limit}`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
  ...
  return (
    <div className="rounded-[10px] bg-white shadow-1 dark:bg-gray-dark dark:shadow-card">
      ...
      <div className="px-4 py-6 md:px-6 xl:px-9">
        ...
        <Link href="/superadmin/master/addcity">
          <button
            type="button"
          ...
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              {stateData.map((city, index) => (
                <tr
                  key={city.id || index} // Prefer country.id if available, fallback to index
                ...
                >
                  ...
                  <td className="px-6 py-4">{city?.country?.title}</td>
                  <td className="px-6 py-4">{city?.state?.title}</td>
                  <td className="px-6 py-4">{city.title}</td>
                  <td className="px-6 py-4">
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        onChange={() => handleStatus(city.status, city.id)}
                        className="sr-only peer"
                        defaultChecked={city.status === "ACTIVE"} // Set defaultChecked based on status
                      />
                      <div
    ...
    peer-checked:bg-blue-500"
                  ...
                  <td className="px-6 py-4">
                    <Link
                      href={`/superadmin/master/addcity?id=${city.id}`}
                    ...
                    <button onClick={() => handleDelete(city.id)} className="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 focus:outline-none dark:focus:ring-red-800">
                      Delete
                    </button>
                  </td>
                </tr>
