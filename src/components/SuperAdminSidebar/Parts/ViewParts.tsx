...
const ViewParts: React.FC = (params: paramsInterface) => {
  ...
  const handleStatus = (status, id) => {
    ...
    try {
      ...
      axios.put(
        `${process.env.API_URL}parts/status/${id}`, // Ensure URL is correct and consistent
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setLoading(true);
      const response = await axios.delete(`${process.env.API_URL}parts/${id}`, {
  ...
  const fetchData = async (page: number) => {
    ...
    const accessToken = localStorage.getItem('accessToken');
    try {
      const result = await axios.get(`${process.env.API_URL}parts/all?page=${currentPage}&limit=${limit}`, {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
  ...
  return (
    <div className="rounded-[10px] bg-white shadow-1 dark:bg-gray-dark dark:shadow-card">
      ...
      <div className="px-4 py-6 md:px-6 xl:px-9">
        ...
        <Link href="/superadmin/master/addparts">
          <button
            type="button"
          ...
          <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
              <tr>
                ...
                <th scope="col" className="px-6 py-3">Parts Name</th>
                <th scope="col" className="px-6 py-3">Parts Code</th>
                <th scope="col" className="px-6 py-3">Status</th>
                <th scope="col" className="px-6 py-3">Action</th>
              </tr>
            </thead>
            <tbody>
              {stateData.map((parts, index) => (
                <tr
                  key={parts.id || index} // Prefer country.id if available, fallback to index
                ...
                >
                  ...
                  <td className="px-6 py-4">{parts.title}</td>
                  <td className="px-6 py-4">{parts.code}</td>
                  <td className="px-6 py-4">
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        onChange={() => handleStatus(parts.status, parts.id)}
                        className="sr-only peer"
                        defaultChecked={parts.status === "ACTIVE"} // Set defaultChecked based on status
                      />
                      <div
    ...
    peer-checked:bg-blue-500"
                  ...
                  <td className="px-6 py-4">
                    <Link
                      href={`/superadmin/master/addparts?id=${parts.id}`}
                      className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"
                    >
                      Edit
                    </Link>
                    <button onClick={() => handleDelete(parts.id)} className="mt-10 text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 focus:outline-none dark:focus:ring-red-800">
                      Delete
                    </button>
                  </td>
                </tr>
